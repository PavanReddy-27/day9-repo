"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.markAllAsRead = exports.markAsRead = exports.getNotifications = void 0;
var Notification_js_1 = __importDefault(require("../models/Notification.js"));
// @desc    Get all notifications for the logged-in user
// @route   GET /api/v1/notifications
// @access  Private
var getNotifications = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var userId, companyId, notifications, error_1;
    var _a;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _b.trys.push([0, 2, , 3]);
                userId = (_a = req.user) === null || _a === void 0 ? void 0 : _a.id;
                companyId = req.companyId;
                if (!userId) {
                    res.status(401).json({ success: false, message: "User not authenticated" });
                    return [2 /*return*/];
                }
                return [4 /*yield*/, Notification_js_1.default.find({ userId: userId, companyId: companyId })
                        .sort({ createdAt: -1 })
                        .limit(50)];
            case 1:
                notifications = _b.sent();
                res.json({ success: true, data: notifications });
                return [3 /*break*/, 3];
            case 2:
                error_1 = _b.sent();
                console.error("Error fetching notifications:", error_1);
                res.status(500).json({ success: false, message: "Server error while fetching notifications" });
                return [3 /*break*/, 3];
            case 3: return [2 /*return*/];
        }
    });
}); };
exports.getNotifications = getNotifications;
// @desc    Mark a specific notification as read
// @route   PATCH /api/v1/notifications/:id/read
// @access  Private
var markAsRead = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var id, userId, companyId, notification, error_2;
    var _a;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _b.trys.push([0, 2, , 3]);
                id = req.params.id;
                userId = (_a = req.user) === null || _a === void 0 ? void 0 : _a.id;
                companyId = req.companyId;
                return [4 /*yield*/, Notification_js_1.default.findOneAndUpdate({ _id: id, userId: userId, companyId: companyId }, { isRead: true }, { new: true })];
            case 1:
                notification = _b.sent();
                if (!notification) {
                    res.status(404).json({ success: false, message: "Notification not found" });
                    return [2 /*return*/];
                }
                res.json({ success: true, data: notification });
                return [3 /*break*/, 3];
            case 2:
                error_2 = _b.sent();
                console.error("Error marking notification as read:", error_2);
                res.status(500).json({ success: false, message: "Server error" });
                return [3 /*break*/, 3];
            case 3: return [2 /*return*/];
        }
    });
}); };
exports.markAsRead = markAsRead;
// @desc    Mark all notifications as read for the user
// @route   PATCH /api/v1/notifications/read-all
// @access  Private
var markAllAsRead = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var userId, companyId, error_3;
    var _a;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _b.trys.push([0, 2, , 3]);
                userId = (_a = req.user) === null || _a === void 0 ? void 0 : _a.id;
                companyId = req.companyId;
                return [4 /*yield*/, Notification_js_1.default.updateMany({ userId: userId, companyId: companyId, isRead: false }, { $set: { isRead: true } })];
            case 1:
                _b.sent();
                res.json({ success: true, message: "All notifications marked as read" });
                return [3 /*break*/, 3];
            case 2:
                error_3 = _b.sent();
                console.error("Error marking all notifications as read:", error_3);
                res.status(500).json({ success: false, message: "Server error" });
                return [3 /*break*/, 3];
            case 3: return [2 /*return*/];
        }
    });
}); };
exports.markAllAsRead = markAllAsRead;
