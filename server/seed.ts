...
const seedDB = async () => {
  try {
    ...
    for (const d of depts) {
      ...
      const deptArr = await Department.create([{ companyId: company._id, locationId: defaultLocationId, name: d, code }], { session });
    ...
    const teamsByDept = {};
    for (const d of depts) {
      const t1Arr = await Team.create([{ companyId: company._id, departmentId: departments[d]._id, name: `${d} Alpha` }], { session });
      const t2Arr = await Team.create([{ companyId: company._id, departmentId: departments[d]._id, name: `${d} Beta` }], { session });
